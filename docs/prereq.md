...
File: [amq-streams-2.4.0-ocp-install-examples.zip](https://access.redhat.com/jbossnetwork/restricted/softwareDetail.html?softwareId=105391&product=jboss.amq.streams&version=2.4.0&downloadType=distributions)

In order to test the setup of AMQ Streams before demoing, and also to monitor AMQ Streams during the demo, we will use the command line clients provided by Red Hat (For more information click [here](https://access.redhat.com/documentation/en-us/red_hat_amq_streams/2.4/html-single/getting_started_with_amq_streams_on_openshift/index#proc-using-amq-streams-str)).  The file link is below:
